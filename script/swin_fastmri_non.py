...
from models import VisionTransformer, ReconNet
...
def trainer(args):
    ...
    ## optim
    loss_function = SSIMLoss().to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_data in train_loader:
            input, target, mean, std, max_value = (
            ...
            for i in range(num_slices):
                ...
                loss = loss_function(output, tar.to(device), max_value.to(device))
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if metric > best_metric:
                    ...
                    torch.save(model.state_dict(), os.path.join(outpath, "swin_mri_reconstruction.pt"))
