...
def trainer(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_data in train_loader:
            ...
            loss_function._data_range = max_value
            ...
            for i in range(num_slices):
                ...
                # forward pass
                inp = input[:, i, ...].unsqueeze(slice_dim).to(device)
                tar = target[:, i, ...].unsqueeze(slice_dim).to(device)
                output = model(inp[0], mask.bool().to(device))
                ...
                loss = loss_function(output_crp, tar, data_range=max_value)
