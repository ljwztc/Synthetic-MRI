...
def trainer(args):
    ...
    for index, batch_data in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        kspace_blur = motion_dg(kspace.permute(0, 2, 3, 1), 'respiration').permute(0, 3, 1, 2)
        # print('information for kspace_blur', kspace_blur.shape, kspace_blur.mean(), kspace_blur.var())
        saved_kspace_blur = kspace_blur.numpy()
        # print(saved_kspace_blur.dtype, saved_kspace_blur.shape)
        ...
        with h5py.File(original_file_name, 'r') as original_file:
            ...
            new_file_dir = Path('/NAS_liujie/liujie/fastMRI/brain/multicoil_train_blur')
            ...
            with h5py.File(new_file_name, 'w') as new_file:
                ...
                new_file.create_dataset('kspace', data=saved_kspace_blur)
                print(f"File '{new_file_name}' has been created with the new 'kspace' dataset.")

        
        # noise_dg = NoiseModel(30)
        # kspace_cpx = convert_to_tensor_complex(kspace)
        ...
        # kspace_noise = torch.view_as_complex(kspace_noise)
        # print(kspace_noise.numpy().dtype, len(kspace_noise.numpy()))
        # print(kspace_noise.numpy().shape)
    ...
    for index, batch_data in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with h5py.File(original_file_name, 'r') as original_file:
            ...
            new_file_dir = Path('/NAS_liujie/liujie/fastMRI/brain/multicoil_val_blur')
            if not new_file_dir.exists():
                new_file_dir.mkdir(parents=True, exist_ok=True)
